...
x_hatP = zeros( 6 , length(ydata)); %Estimated xHat plus
y_hatP = zeros( 5 , length(ydata)); %Estimated yHat plus
...
P_vecP = zeros(6,6, length(tvec));
 ...
 x_hatP(:,1) = [10;0;1.57079632679490;-60;0;-1.57079632679490];
 
 %Initializing P to be somewhat Large
 P_vecP(:,:,1) = 10000* eye(6);
 ...
 for ii = 1:length(tvec)-1
    ...
    eytil_kp1 = ydata(:,ii+1)  -  yHat_kp1Min;
...
%% Plotting States
figure()
sgtitle('EKF - Given Dataset Performance')
hold on;
for k = 1:6
    ax(k) = subplot(3,2,k);
end
...
xlabel('time (s)');
ylabel('$e_{\xi_g}$','Interpreter','latex');
title('$e_{\xi_g}$','Interpreter','latex')
...
xlabel('time (s)')
ylabel('$e_{\eta_g}$','Interpreter','latex');
title('$e_{\eta_g}$','Interpreter','latex');
axis([0 100 -10 50])
...
xlabel('time (s)')
ylabel('$e_{\theta_g}$','Interpreter','latex');
title('$e_{\theta_g}$','Interpreter','latex');
axis([0 100 -3 3])
...
xlabel('time (s)')
ylabel('$e_{\xi_a}$','Interpreter','latex');
title('$e_{\xi_a}$','Interpreter','latex');
axis([0 100 -60 140])
...
xlabel('time (s)')
ylabel('$e_{\eta_a}$','Interpreter','latex');
title('$e_{\eta_a}$','Interpreter','latex');
axis([0 100 -105 105])
...
xlabel('time (s)')
ylabel('$e_{\theta_a}$','Interpreter','latex');
title('$e_{\theta_a}$','Interpreter','latex');
axis([0 100 -4 4])
