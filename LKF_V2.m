...
%  Omega_tilde = eye(6).*dt; % should it be multiplied by dt?
 Omega_tilde = eye(6); % should it be multiplied by dt?
 for idx = 2:length(x_nom(1,:))
     % dx tilde minus
     dx_tilde(:,idx) = F_tilde(tvec(idx-1), dt)*dx_tilde(:,idx-1);% + G_tilde(tvec(idx-1), dt)*uVec;
     %P minus at K+1
     P((idx-1)*6+1:6*idx,:) = F_tilde(tvec(idx-1), dt)*P((idx-2)*6+1:6*(idx-1),:)*F_tilde(tvec(idx-1), dt)' + Omega_tilde*Qtrue*Omega_tilde';
     
     % Measurement update
     % Note I put ydata - 1 because the first data point was nans and i
     % deleted it
     dy(:,idx) = ydata(:,idx-1) - h(x_nom(:,idx));
